...
homedir = os.path.dirname(os.path.realpath(__file__)).split('models')[0]
...
@weave.op()
def _write_io_codes(spec, client, model_name):
    chunks = spec.split()
    if len(chunks) == 1:
...
@weave.op()
def _verify_logic(prompt: str, client, model_name):
    try:
        chunks = prompt.split()
...
@weave.op()
def _give_hint(problem_num: str, client, model_name):
    try:
        problem_num = str(int(problem_num))
    except Exception:
        return f"Unknown problem number {problem_num}"
